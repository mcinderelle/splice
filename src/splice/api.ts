...
export function createSearchRequest(query: string): SpliceSearchRequest {
  return {
    ...
    variables: {
      attributes: [],
      // Use the free-text GraphQL query for broad matching; keep filepath for exact filename hits
      query,
      filepath: query,
      legacy: true,
      limit: 50,
...
export type SpliceSample = {
  ...
  // Optional COSO playback metadata (present for many loop assets)
  coso_playback_metadata?: {
    psOffset?: number | null,
    numBars?: number | null,
